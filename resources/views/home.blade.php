...
@section('content')
    ...
    <form method="POST" action="{{ route('logout') }}" class="text-right">
        @csrf
        <button type="submit"
            class="bg-red-600 text-white px-4 py-2 rounded shadow-md hover:bg-red-700 transition-all hover:scale-105 focus:outline-none focus:ring-2 focus:ring-red-400 dark:bg-red-700 dark:hover:bg-red-800">
            Logout
        </button>
    </form>
    ...
    <div class="max-w-4xl mx-auto px-6 py-10">
        <div class="flex justify-between items-center mb-8">
            <div class="flex justify-center items-center h-full">
                <h1 class="text-center text-4xl font-semibold text-gray-800 dark:text-gray-100 tracking-tight">Laravel Browser</h1>
            </div>
        ...
        @if(!$query)
            <div class="alert alert-warning mb-4">
                Inserisci un termine di ricerca per ottenere i risultati.
            </div>
        @elseif($message)
        ...
        @if($results->isNotEmpty())
            <h2 class="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-4">Risultati per <span
                    class="text-blue-600 dark:text-blue-400">"{{ $query }}"</span></h2>
            <ul class="space-y-6">
        ...
        @else
            <p class="mt-4 text-gray-500 dark:text-gray-400">Nessun risultato trovato per "<strong>{{ $query }}</strong>".</p>
        @endif
    </div>
